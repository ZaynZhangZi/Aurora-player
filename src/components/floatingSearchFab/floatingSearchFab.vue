<template>
  ...
  <div class="fixed inset-x-0 top-0 z-50 pointer-events-none">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div
        ref="shell"
        :class="expanded ? 'rounded-full pl-0 pr-4 sm:pr-6 h-12 overflow-visible' : 'h-12 w-12 rounded-full overflow-hidden'"
        class="mt-3 grid items-center border border-white/10 shadow-lg ring-1 ring-white/10 bg-white/10 backdrop-blur-md backdrop-saturate-150 [grid-template-columns:48px_1fr] pointer-events-auto"
        ...
        <div v-show="expanded" ref="content" class="col-[2/3] min-w-0 w-full flex items-center justify-between gap-4">
          ...
          <div v-if="showAuth" class="flex items-center space-x-4">
            ...
            <div
              v-else
              class="relative"
              @mouseenter="onAvatarTriggerEnter"
              @mouseleave="onAvatarTriggerLeave"
            >
              <button
                class="flex items-center gap-2 rounded-full text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
                type="button"
                :aria-expanded="showAvatarMenu"
                @click="onAvatarToggleClick"
              >
                <img
                ...
                <ChevronDownIcon
                  ...
                  :class="showAvatarMenu ? 'rotate-180' : 'rotate-0'"
                />
              </button>

              <div
                v-if="showAvatarMenu"
                class="absolute right-0 mt-2 w-40 rounded-2xl bg-zinc-900/90 py-2 text-sm text-white shadow-lg ring-1 ring-white/10 backdrop-blur-lg z-20"
                @mouseenter="onAvatarMenuEnter"
                @mouseleave="onAvatarMenuLeave"
              >
                <button
...
import { XMarkIcon, Bars3Icon, ChevronDownIcon, ArrowRightOnRectangleIcon } from '@heroicons/vue/24/outline'
...
function cancelAvatarMenuClose () {
  if (avatarMenuCloseTimer) {
    clearTimeout(avatarMenuCloseTimer)
    avatarMenuCloseTimer = null
  }
}

function openAvatarMenu () {
  cancelAvatarMenuClose()
...
function scheduleAvatarMenuClose () {
  closeAvatarMenu()
}

function onAvatarTriggerEnter () {
  openAvatarMenu()
}

function onAvatarTriggerLeave () {
  scheduleAvatarMenuClose()
}

function onAvatarMenuEnter () {
  openAvatarMenu()
}

function onAvatarMenuLeave () {
  scheduleAvatarMenuClose()
}

function onAvatarToggleClick () {
...
const emit = defineEmits(['update:modelValue', 'expand', 'collapse', 'toggle', 'search', 'signin', 'signup', 'logout'])
...
watch(expanded, (val) => {
  if (!val) closeAvatarMenu(true)
})

watch(isLoggedIn, (loggedIn) => {
  if (!loggedIn) closeAvatarMenu(true)
})
